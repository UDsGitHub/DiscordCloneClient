...
const ServerAvatar = ({ server, active, setActive, index }: ServerAvatarProps) => {
  ...
  return (
    <div
      className={`w-full flex justify-center mb-2 relative [&>button]:hover:rounded-2xl [&>button]:hover:bg-purple-500 ${
        index !== active && "[&>span]:hover:h-6"
      }`}
    >
      <span
        className={`bg-grey-400 w-3 ${
          index === active ? "h-12" : "h-2"
        } duration-300 rounded-full absolute -left-[28%] top-1/2 -translate-y-1/2`}
      ></span>
      <button
        className={`h-12 w-12 duration-300 flex justify-center items-center ${
          index === active
